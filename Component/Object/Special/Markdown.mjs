...
export default class TSNJS_Markdown extends BASE_OBJECT {
	constructor(Text) {
		super();
		this.name = "TSNJS_Markdown";
		this.Delete_Delay = 0;
		...
		// TSNJS v3.0 Notice: This uses an outdated version of MarkTSNDL, the following code will need to be replaced and ported from TSN_Kronos
		// Man-made horrors.
		const H6 = /(?<=^###### ).*/g
		const H5 = /(?<=^##### ).*/g;
		const H4 = /(?<=^#### ).*/g;
		...
		for (let Index = 0; Index < Lines.length; Index++) {
			if (Lines[Index] == "```" && !inCode) { inCode = true; Code += "<Codeblock>"; continue; }
			else if (Lines[Index] == "```") { inCode = false; Code += "</Codeblock>"; continue; };
			...
			SmallCodes = Lines[Index].match(SmallCode); if (SmallCodes != null) {
				for (let Match = 0; Match < SmallCodes.length; Match++) {
					Lines[Index] = Lines[Index].replace(SmallCodes[Match], `<code>${SmallCodes[Match].replace("`", "").replace("`", "")}</code>`);
				};
			};
...
window.TSNJS.Component.Object.Markdown = TSNJS_Markdown;